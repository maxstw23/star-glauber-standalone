		...
		for(Int_t it=0; it<3; it++){ // vary total cross section by 5 % in it=1,2
			...
			Double_t distance = 1000.0;
                        for(Int_t im=0; im<nbin; im++){
                                ...
                                if( isCentOk && bin < ncent ){
                                        std::cout << Form("%2.2f - %2.2f (%%) :  M > %4d (im=%3d, M=%1.1f, bin=%4d) (sum, total, fraction>cut) = (%1.3f, %1.3f, %1.3f>%1.3f)",
                                                        TMath::Abs(centralityMin[bin]*scale), TMath::Abs(centralityMax[bin]*scale), Mint, im, (Double_t)Mint, bin, sum, nevent, R, fCentBinCut) << std::endl;
                                        centBin[i][it][bin] = (Double_t)Mint ;
                                        bin++;
                                        distance=1000.0;
                                }
